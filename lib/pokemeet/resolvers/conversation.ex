defmodule Pokemeet.Resolvers.ConversationResolver do
  import Ecto.Query
  alias Ecto.Multi
  alias Pokemeet.Service.ConversationService
  alias Pokemeet.{Repo, Conversation, ConversationUser, User}

  def list(_parent, _args, %{context: %{current_user: current_user}}) do
    {:ok, ConversationService.all_for_user(current_user)}
  end
  ...
  def show(_parent, args, %{context: %{current_user: current_user}}) do
    case ConversationService.find_for_user(args.id, current_user) do
      nil -> {:error, "Not Found"}
  ...
  def create(_parent, args, %{context: %{current_user: current_user}}) do
    ...
    if existing = ConversationService.find_for_users(user_ids) do
     {:ok, existing}
    else
      title =
        User
